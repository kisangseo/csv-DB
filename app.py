    ...
    if tenant_id and client_id and client_secret:
        ...
        try:
            ...
            if token_resp.status_code >= 400:
                print(f"[DV EMAIL] Token request failed status={token_resp.status_code} body={token_resp.text[:800]}")
            ...
            error_text = str(graph_exc)
            print(f"[DV EMAIL] Graph ingest failed: {error_text}")
...
def get_dv_files_container():
    if not CONNECTION_STRING:
        raise RuntimeError("Missing AZURE_STORAGE_CONNECTION_STRING env var.")
    return ContainerClient.from_connection_string(CONNECTION_STRING, DV_PDF_BLOB_CONTAINER)
...
@app.route("/dv-pdf/files/upload", methods=["POST"])
